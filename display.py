...
# Dessine le graphique à la fin de la simulation
def draw_graph(screen, history):
    ...
    for i in range(len(history) - 1):
        ...
        data = [(history[i][0], history[i+1][0], (255, 255, 255)), # Proies
                (history[i][1], history[i+1][1], (255, 100, 100)), # Preds
                (history[i][2]//10, history[i+1][2]//10, (100, 255, 100))] # Herbe
        for v1, v2, color in data:
            y1 = base_y - (v1 * height // max_val)
            y2 = base_y - (v2 * height // max_val)
            pygame.draw.line(screen, color, (x1, y1), (x2, y2), 2)

# Écran de fin affichant les statistiques globales
def show_bilan(screen, font_l, font_s, texte_bilan, history):
    ...
    while running:
        ...
        msg_exit = "Appuyez sur une touche ou fermez la fenêtre pour quitter"
        screen.blit(font_s.render(msg_exit, True, (150, 150, 150)), (230, 560))
        pygame.display.flip()
...
def display_process(shared_state, msg_queue, lock):
    ...
    history = [] # Stocke les points du graphique
    ...
    while True:
        ...
        # Analyse des messages texte venant des agents
        while not msg_queue.empty():
            try:
                msg = msg_queue.get_nowait()
                if isinstance(msg, tuple) and msg[0] == "BILAN":
                    show_bilan(screen, font_l, font_s, msg[1], history); return
                # Compte les événements pour les statistiques en temps réel
